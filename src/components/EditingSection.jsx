...
const EditingSection = ({ processedData, onEditingComplete }) => {
  ...
  useEffect(() => {
    if (processedData?.formattedDocument) {
      ...
      setTempPageNumber(String(data.startPageNumber || 1))
      ...
      initialSelection.startPageNumber = true
  ...
  const handleReset = () => {
    if (originalData) {
      ...
      setTempPageNumber(String(originalData.startPageNumber || 1))
    }
  }

  const handlePageNumberSubmit = () => {
    const pageNum = parseInt(tempPageNumber, 10)
    if (!isNaN(pageNum) && pageNum > 0) {
      handleSectionEdit("startPageNumber", String(pageNum))
    }
    setShowPageNumberInput(false)
  }
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-3">
            <div className="relative">
              ...
              {showPageNumberInput && (
                <div className="absolute top-full mt-2 z-10 bg-white border border-gray-300 rounded-lg shadow-lg p-3">
                  <input
                    type="number"
                    value={tempPageNumber}
                    onChange={(e) => setTempPageNumber(e.target.value)}
                    placeholder="Enter page number"
                    className="w-32 p-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    min="1"
                  />
                  <div className="flex justify-end space-x-2 mt-2">
                    <button
                      onClick={() => setShowPageNumberInput(false)}
                    ...
                    <button
                      ...
                      className="px-3 py-1 text-sm text-white bg-primary-500 hover:bg-primary-600 rounded"
                    >
                      Save
                    </button>
                  </div>
...
const DocumentPreview = ({ data, selectedSections, onEdit }) => {
  ...
  const getHTML = (field) => {
    ...
    if (field === "startPageNumber") return { __html: String(data.startPageNumber || 1) }
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-8 max-h-96 overflow-y-auto">
      <div className="space-y-6 text-sm">
        ...
        {selectedSections.keywords && data.keywords && data.keywords.content && (
          <div>
            <h2 className="font-bold text-sm uppercase mb-2">KEYWORDS</h2>
            <p
              className={`text-justify leading-relaxed cursor-pointer hover:bg-blue-50 p-2 rounded transition-colors ${
                editingField === "keywords" ? "bg-blue-100 border border-blue-300" : ""
              }`}
              contentEditable={editingField === "keywords"}
              suppressContentEditableWarning={true}
              onClick={() => handleFieldClick("keywords")}
              onBlur={(e) => handleFieldBlur(e, "keywords")}
              onKeyDown={(e) => handleKeyDown(e, "keywords")}
              style={{ minHeight: "2em" }}
              dangerouslySetInnerHTML={getHTML("keywords")}
            />
          </div>
        )}
        {data.sections?.map(
              ...
              {Array.isArray(data.references.content) ? (
                data.references.content.map((ref, index) => (
                  <p key={index} className="text-justify">
                    {index + data.startPageNumber}. {typeof ref === "object" && ref.text ? ref.text : String(ref)}
                  </p>
                ))
              ) : (
                <p
        ...
        <div className="text-center border-t pt-4 text-xs text-gray-600">
          Page {data.startPageNumber} | www.rsisinternational.org
        </div>
      </div>
    </div>
  )
